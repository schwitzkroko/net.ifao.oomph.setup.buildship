   ...
   extends DtdInfoFrame
{
   ...
   private Data data; //  @jve:decl-index=0:
    ...
    * @param checkBoxSelfDefined1 TODO (brod) add text for param checkBoxSelfDefined 
    * @return TODO (brod) add text for returnValue 
    * 
    * @author brod 
    */
   private boolean checkChange(XmlObject xml, String attribute, Button checkBoxSelfDefined1)
   {
      boolean bOk = xml.getAttribute(attribute).equalsIgnoreCase("true");
      if (checkBoxSelfDefined1.getSelection() != bOk) {
         xml.setAttribute(attribute, checkBoxSelfDefined1.getSelection() ? "true" : null);
         return true;
      }
      return false;
   }
   ...
   private Image getImage(boolean bOpened, XmlObject xml, Display device)
   {
      ...
      if (xml.getAttribute("type").length() == 0)
         image = Data.getImageFolder(bOpened, xml, device);
      else
         image = Data.getTextFolder(xml, device);
    ...
    * @param dtdStatus 
    */
   private void loadData(DtdStatus dtdStatus)
   {
      try {
   ...
   {
      try {
         DtdStatus dtdStatus = new DtdStatus();
         ...
         textProvider.setText(xml.getAttribute("provider"));

         loadSash(xml, "1", sashForm);
         loadSash(xml, "2", sashForm2);

         // add Tree Adapter for openClose
         org.eclipse.swt.events.TreeAdapter treeAdapter = new org.eclipse.swt.events.TreeAdapter()
         {
            @Override
            ...
            public void treeExpanded(org.eclipse.swt.events.TreeEvent e)
            {
               try {
                  ...
                  TreeItem source = (TreeItem) e.item;
                  source
                        .setImage(getImage(true, (XmlObject) source.getData(), source.getDisplay()));
               }
               catch (Exception ex) {
                  ex.printStackTrace();
               }
         ...
         data = new Data(dtdStatus, "c:\\arctic\\Workspace\\arctic");
