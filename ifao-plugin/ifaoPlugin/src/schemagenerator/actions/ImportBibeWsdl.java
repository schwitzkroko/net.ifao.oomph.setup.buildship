...
/**
 * Class ImportBibeWsdl
 *
 * <p>
 * Copyright &copy; 2009, i:FAO Group GmbH
 * @author kaufmann
 */
public class ImportBibeWsdl
{
   ...
   /**
    * Constructor ImportBibeWsdl
    *
    *
    * @author kaufmann
    */
   public ImportBibeWsdl(String psBaseDir, String psWsdlFile, String psJavaCommunication)
      throws Exception
   {
   ...
   {
      String sOldWsdlFileName = Util.getProviderDataPath(_sBaseDir, "de/bahn/www/data.wsdl");
   ...
   private void replaceArrays(Hashtable<String, List<String>> phtComplexTypes, XmlObject pXmlObject)
   {
      ...
      if ((sType.length() > 0) && (phtComplexTypes.get(sType) != null)) {
         ...
         XmlObject complexType = new XmlObject("<xsd:complexType/>").getFirstObject();
         pXmlObject.addElementObject(complexType);
         XmlObject sequence = new XmlObject("<xsd:sequence/>").getFirstObject();
         complexType.addElementObject(sequence);
         arrayItem.setAttribute("maxOccurs", "unbounded");
         arrayItem.setAttribute("minOccurs", "0");
         arrayItem.setAttribute("name", "item");
               ...
               if (schema.getObjects("").length > 0) {
                  ...
                  for (String sNamespace : namespacesUsed) {
                     ...
                     if (sNamespaceURI == null) {
                        sNamespaceURI = schemaNamespaces.get(sNamespace);
                     }
                     if (sNamespaceURI == null) {
                        sNamespaceURI = sSchemaTargetNamespace;
                     }
                     if (sNamespaceURI != null) {
                  ...
                  if (psBinding != null && psBinding.length() > 0) {
                     bChanged |=
                        Util.updateFile(
                              Util.getProviderDataPath(_sBaseDir, sPathForDataXsd
                                    + "dataBinding.xml"), psBinding, _sbResult);
                     if (CorrectDatabindingXsd
                           .correctDataBinding(
                                 Util.getProviderDataFile(_sBaseDir, sPathForDataXsd
                                       + "dataBinding.xml"), "")) {
                        bChanged = true;
                     }
                  }
         ...
         if (sType.length() > 0) {
            if (pSchema.findSubObject("element", "name", sType) == null) {
               if (phtMessages.containsKey(sType)) {
                  String sElement =
                     "<xsd:element name=\"" + sType + "\" type=\"" + phtMessages.get(sType)
                           + "\"/>";
   ...
   {
      String sBasePath = Util.getPathFromNamespace(psNamespaceURIBase, "www.bahn.de");
      String[] basePathParts = sBasePath.split("[\\\\/]");
      String sPath = Util.getPathFromNamespace(psNamespaceURI2, "www.bahn.de");
      ...
      for (int j = i; j < pathParts.length; j++) {
         sbNewPath.append(pathParts[j]).append("/");
      }
   ...
   /**
    * Method updateCastorBuilderProperties adds the code for the castorbuilder.properties file for 
    * the WebService passed. Field _namespacesCreated must contain all namespaces (=sub-packages) 
    * used by the webservice
    * @author kaufmann
    */
   private void updateCastorBuilderProperties()
   {
      ...
      for (String sNamespace : _namespacesCreated) {
         String sPackage =
            Util.getPathFromNamespace(sNamespace, "www.bahn.de").replaceAll("[\\\\/]", ".");
         sPackage = _namespacePackage.get(sNamespace).replaceAll("lib.provider.*?data.", "");
      ...
      for (String sNamespace : _namespacesCreated) {
         String sPathForDataXsd =
            Util.getProviderDataFile(_sBaseDir,
                  Util.getPathFromNamespace(sNamespace, "www.bahn.de")).getAbsolutePath();
         Util.updateFile(sPathForDataXsd + "/castorbuilder.properties",
   ...
   private void updateJavaCommunication(String psWsdl)
   {
      ...
      writeBahnCommunication("B2BRetrieval", sJavaCommunication);
   }
   ...
   private String createJavaCommunication(String psWsdl)
   {
      ...
      sbJava.append(getInnerMethod(hsExceptions, "Bahn"));
   ...
   private String getInnerMethod(HashSet<String> phsExceptions, String psProvider)
   {
      ...
      sb.append("         }\n");
      sb.append("         throw new ").append(psProvider)
            .append("Exception(psResponse, log, false);\n");
         ...
         if (operation != null) {
            ...
            if (sMessageOutput.length() > 1) {
               ...
               if (messageInput != null) {
                  ...
                  if (bIsSoapAction) {
                     sbJava.append("      setSOAPAction(\"").append(sSoapAction).append("\");\n\n");
                  } else {

                     // if there is no soapAction ... add the method
                     if (sInputNameSpace.length() > 0) {
                        sbJava.append("      sbRequest.append(\"<m:").append(psMethod)
                              .append(" xmlns:m=\\\"").append(sInputNameSpace)
                              .append("\\\">\");\n");
                     } else {
                        sbJava.append("      sbRequest.append(\"<m:").append(psMethod)
                              .append(">\");\n");
                  ...
                  if ((sInputPackage.length() > 0)
                        && (sbJava.indexOf("import " + sInputPackage) < 0) && bAddImport) {
                     sbJava.insert(0, "import " + sInputPackage + ".*;\n");
                  }
    ...
    * @author kaufmann
    */
   private void writeBahnCommunication(String psService, String psJavaCommunication)
   {
      // get the imports
      String sImport = "";
      ...
      if ((sSource.length() > 0) && (psJavaCommunication.length() > 0)) {
         ...
         while (psJavaCommunication.startsWith("import ")) {
            ...
            sPackage = sImport.substring(sImport.indexOf(" ") + 1, sImport.lastIndexOf(".*"));
   ...
   private String encapsulateCommunication(String sSource, String psName)
   {
      ...
      StringBuilder sbHeader = new StringBuilder();
      sbHeader.append("   // The following lines are automacically generated with the\n");
