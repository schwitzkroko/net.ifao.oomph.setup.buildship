...
public class CommonClassCorrector
{
   ...
   class SchemaFiles
   {
      ...
      public SchemaFiles(File file2, String psNameSpace)
      {
         file = file2;
         _schemaXsd =
            new XmlObject("<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" xmlns=\""
                  + psNameSpace + "\" targetNamespace=\"" + psNameSpace
      ...
      public String addTargetNameSpace(String psTargetNameSpace, String psRelativePath)
      {
         ...
         XmlObject xmlImport =
            _schemaXsd.createObject("xs:import", "namespace", psTargetNameSpace, true);
         ...
         while (true) {
            ...
            if (attribute.length() == 0) {
               ...
               if (createObject == null) {
                  createObject =
                     _schemaXsd.createObject(_schemaXsd.getNameSpace() + ":import", "namespace",
                           sXmlns, true);
                  createObject.setAttribute("schemaLocation",
                        Utils.getRelativePath(file, pSchemaFile));
   ...
   class CommonType
   {
      ...
      public CommonType(SchemaFiles pSchema, XmlObject copy, String withoutAnnotation,
                        SchemaFiles file)
      {
         _schema = pSchema;
         this.copy = copy;
      ...
      public void moveTo(SchemaFiles schemaFiles, String psParentName)
      {
         ...
         for (SchemaFiles file : lstFiles) {
            XmlObject subObject =
               file._schemaXsd.findSubObject(newXmlObject.getName(), "name", sName);
            if (subObject != null) {
               file._schemaXsd.deleteObjects(subObject);
               String sNs =
                  file.addTargetNameSpace(sTargetNamespace, schemaFiles.getRelativePath(file));
         ...
         println("- Move (" + sFiles + ") " + sName + " to (" + sTargetNamespace + ") "
               + sReplaceName);
      }

      private void removeTypeReferences(XmlObject newXmlObject, SchemaFiles pSchemaFrom,
                                        SchemaFiles pSchemaTo)
      {
      ...
      for (File subDirectory : rootDir.listFiles()) {
         ...
         if (schemaGroupFile.exists()) {
            ...
            SchemaFiles groupSchemaFiles =
               new SchemaFiles(new File(subDirectory, "schema.xsd"), sNameSpace
            ...
            while (st.hasMoreTokens()) {
               ...
               if (sLine.contains("=")) {
                  ...
                  while (stItems.hasMoreTokens()) {
                     String sPath =
                        subDirectory.getName() + File.separator + stItems.nextToken()
                              + File.separator;
      ...
      for (SchemaFiles schemaFiles : lstSubSchemas) {
         for (XmlObject complexType : schemaFiles._schemaXsd.getObjects("complexType")) {
            if (!containsInnerReferences(complexType)) {
               lstCommonTypes.add(new CommonType(schemaFiles, complexType, withoutAnnotation(
                     complexType.copy()).toString(), schemaFiles));
            }
         }
      }
      ...
      for (SchemaFiles xmlObject : lstSchemaFiles) {
         for (XmlObject simpleType : xmlObject._schemaXsd.getObjects("simpleType")) {
            ...
            if (sName.length() > 0) {
               ...
               if (commonType == null) {
                  htNames.put(sName, new CommonType(xmlObject, simpleType, withoutAnnotation,
                        xmlObject));
               } else {
         ...
         if (file.isDirectory() && piCount > 0) {
            ...
            try {
               SchemaFiles schemaFiles =
                  new SchemaFiles(file, new XmlObject(file).getFirstObject());
