...
public class SwtBestWestern
   extends SwtBase
{

   private final static String IMPORT_DIR = "net/ifao/providerdata/bestwestern";

   private ICorrector _corrector = new BestWesternCorrector();

   private Text textArea = null;

   private Text textAreaCorrections = null;

   public SwtBestWestern(Composite parent, int style)
   {
      super(parent, style);
      initialize();
   }
      ...
      if (sImportDir.length() > 0) {
         ...
         if (bCheckBoxXsd) {
            ...
            if (bCreatePackages) {
               ...
               ImportXml.startBuild(sImportDir, bIgnoreSimpleTypes, true, _corrector, "", true,
                     bDateTimeHandler);
